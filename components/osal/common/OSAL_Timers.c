...
/*********************************************************************
* @fn      osal_get_timer_active
*
* @brief
*
* @param   uint8_t task_id - task id of timer to check
*
* @return  Return the timer's flag if found, zero otherwise.
*/
uint16_t osal_get_timer_active(uint8_t task_id)
{
	uint16_t task_timer = 0x0000;
	uint16_t event = 0x0000;
	uint8_t i;
	for(  i = 0; i < 16; i++ )
	{
		event = 0x0001<<i;
		if( osal_get_timeoutEx( task_id,event))
			task_timer|= (0x0001<<i);
	}
	return task_timer;
...
* @return  none
*********************************************************************/
void osal_adjust_timers( uint32_t time )
{
  uint32_t eTime = 0;

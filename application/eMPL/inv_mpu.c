...
//}
#define log_i(a) simple_uart_putstring((const uint8_t *)a)		//打印信息
#define log_e(a) simple_uart_putstring((const uint8_t *)a)	//打印信息
...
int mpu_set_dmp_state(unsigned char enable)
{
  ...
  bool res;
    ...
    if (enable) {
        ...
        tmp = 0;
        res = i2c_write(st.hw->addr, 0x23, 1, &tmp);
...
unsigned char mpu_dmp_init(void)
{
	...
	{	
		...
		res = dmp_load_motion_driver_firmware();	//加载dmp固件 800ms
		if(res)
			return 4;
		...
		res = dmp_set_fifo_rate(DEFAULT_MPU_HZ);	//设置DMP输出速率(最大不超过50Hz)
		if(res)
			return 7;
		
		res = run_self_test();		//自检 360ms
		//if(res)
			//return 8; 
